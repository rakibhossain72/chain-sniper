...
using ChainSniper with dynamic filtering. Rules can be pushed to the running
listener via Redis Pub/Sub without restarting the bot.
...
Architecture:
    ...
    4. Rules are dynamically added to the Filter in real-time
...
async def start_redis_rule_listener(
    ...
    try:
        ...
        logger.info(
            "Push rules using: python examples/push_redis_rule.py [action]"
        )
