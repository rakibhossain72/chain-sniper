...
async def main():
    ...
    # Subscribe for node side logs
    log_filter.subscribe(address="0x55d398326f99059fF775485246999027B3197955")
    
    my_strategy = CustomStrategy(logger)
    
    # Configure Pipeline with split filters
    pipeline = Pipeline(tx_filter=tx_filter, log_filter=log_filter, strategy=my_strategy)
    
    # Hook the pipeline instance's methods up to sniper
    sniper.on_block(pipeline.process_block)
    sniper.on_event(pipeline.process_log)
    
    # Also pass filters to sniper for built-in filtering
    sniper.filter(tx_filter=tx_filter, log_filter=log_filter)

    logger.info("Starting sniper with Pipeline Split Filters Strategy...")
    await sniper.start()
