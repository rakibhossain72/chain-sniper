...
class WebSocketListener:
    ...
    async def _subscribe_logs(self, ws) -> None:
        for flt in self._log_filters:
            ...
            self.logger.info("Subscribed to logs     → sub_id=%s  filter=%s", sub_id, params)
            ...
            if self.block_detail == BlockDetail.FULL_BLOCK and self._ws:
                ...
                if block_hash:
                    try:
                        ...
                        self.logger.warning("Could not fetch full block %s: %s", block_hash, exc)
            await self._emit("block", result)

        elif event == "log":
            await self._emit("log", result)
